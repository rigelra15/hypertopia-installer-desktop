...
function variantNameFor(accessTypes) {
  const sorted = [...(accessTypes || [])].map((s) => String(s || '').toLowerCase().trim()).sort()
  const key = sorted.join(',')
  switch (key) {
    case 'standalone': return 'Standalone'
    case 'pcvr': return 'PCVR'
    case 'qgo': return 'Quest Games Optimizer (QGO)'
    case 'pcvr,standalone': return '[Bundle] Standalone + PCVR'
    case 'qgo,standalone': return '[Bundle] Standalone + QGO'
    case 'pcvr,qgo': return '[Bundle] PCVR + QGO'
    case 'pcvr,qgo,standalone': return '[Bundle] Standalone + PCVR + QGO'
    default: return null
  }
}
      ...
      try {
        ...
        if (ordersRes.ok) {
          ...
          if (userOrders.length > 0) {
            ...
            const directTxs = (baseProfile.transactions || []).filter(
              (t) => t.source !== 'shopee'
            )
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-1">
                  {profile.transactions.slice(0, 5).map((tx) => {
                    const items = Array.isArray(tx.items) && tx.items.length > 0
                      ? tx.items
                      : [{
                          productName: tx.product || tx.accessTypes?.join(', ') || 'Transaction',
                          variantName: null,
                          productImage: null,
                    ...
                    const totalPrice = tx.totalPrice ?? items.reduce((s, i) => s + (i.subtotal || 0), 0)

                    const statusConfig = {
                      redeemed: { label: 'Selesai', icon: 'mdi:check-circle', cls: 'bg-[#0081FB]/15 text-[#0081FB] border-[#0081FB]/30' },
                      success: { label: 'Berhasil', icon: 'mdi:check-circle', cls: 'bg-[#0081FB]/15 text-[#0081FB] border-[#0081FB]/30' },
                      paid: { label: 'Berhasil', icon: 'mdi:check-circle', cls: 'bg-[#0081FB]/15 text-[#0081FB] border-[#0081FB]/30' },
                      pending: { label: 'Diproses', icon: 'mdi:clock-outline', cls: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' },
                    }
                    const status = statusConfig[tx.status] || { label: tx.status || 'Unknown', icon: 'mdi:help-circle', cls: 'bg-gray-500/20 text-gray-400 border-gray-500/30' }
                      ...
                      >
                        ...
                        <div className="px-3 py-2.5 bg-gray-50 dark:bg-white/5 border-b border-gray-200 dark:border-white/10 flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2 min-w-0">
                            <div className="p-1 bg-white dark:bg-[#0a0a0a] rounded-md border border-gray-200 dark:border-white/10 shrink-0">
                              <Icon icon="mdi:package-variant-closed" className="h-3.5 w-3.5 text-[#0081FB]" />
                            </div>
                            <div className="flex flex-col min-w-0">
                              <span className="text-[10px] text-gray-500 dark:text-white/40 leading-tight">Order ID</span>
                          ...
                          <div className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border flex items-center gap-1 shrink-0 ${status.cls}`}>
                            ...
                            <div key={idx} className="flex gap-3">
                              <div className="w-14 h-14 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 overflow-hidden shrink-0 flex items-center justify-center">
                                {item.productImage ? (
                                  ...
                                  <Icon icon="mdi:package-variant-closed" className="h-6 w-6 text-gray-300 dark:text-white/20" />
                                )}
                              </div>
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-1.5 text-[11px] text-gray-500 dark:text-white/50">
                                  ...
                                  {item.priceAtPurchase > 0 && (
                                    <>
                                      <span>•</span>
                                      <span>Rp {Number(item.priceAtPurchase).toLocaleString('id-ID')}</span>
                                    </>
                                  )}
                                </div>
                              </div>
                        ...
                        <div className="px-3 py-2.5 border-t border-gray-200 dark:border-white/10 bg-gray-50/50 dark:bg-white/5 flex items-center justify-between gap-2">
                          <div className="flex flex-col">
                            <span className="text-[10px] text-gray-500 dark:text-white/40">Total Pesanan</span>
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 px-4 py-6 text-center">
          <Icon icon="mdi:history" className="h-8 w-8 mx-auto text-gray-300 dark:text-white/20 mb-2" />
